...
export default function JoinPage() {
  ...
  const { data: invite, isLoading, isError } = useQuery<InviteInfo>({
    ...
    queryFn: () => api.get(`/households/invite/${token}`).then((r) => r.data),
    enabled: !!token,
    retry: false,
  });
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 p-4">
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="space-y-1 text-center">
          ...
          {isLoading && <p className="text-zinc-500">Loading invite...</p>}
          {isError && <p className="text-zinc-500">This invite link is invalid or has expired.</p>}
          {invite && (
            <p className="text-zinc-500">
              You've been invited to join <span className="font-semibold text-zinc-800">{invite.household_name}</span>
            </p>
          )}
        </CardHeader>
          ...
          {invite && (
            <>
              <Button
                ...
                onClick={() => {
                  window.location.href = `/api/auth/google?invite_token=${token}`;
                }}
              >
                <GoogleIcon />
                Join with Google
